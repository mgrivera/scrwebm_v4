...
angular.module("scrwebM").controller("TiposSiniestroController", ['$scope', '$stateParams', '$meteor',
  function ($scope, $stateParams, $meteor) {
      ...
      // ---------------------------------------------------------
      // subscriptions ...
      $meteor.subscribe('tiposSiniestro').then(function (subscriptionHandle) {
      ...
      $scope.save = function () {
          ...
          var editedItems = _.filter($scope.tiposSiniestro, function (item) { return item.docState; });
              ...
              if (item.docState != 3) {
                  ...
                  if (!isValid) {
                      TiposSiniestro.simpleSchema().namedContext().validationErrors().forEach(function (error) {
                          errores.push("El valor '" + error.value + "' no es adecuado para el campo '" + TiposSiniestro.simpleSchema().label(error.name) + "'; error de tipo '" + error.type + "'.");
                      });
                  }
              }
          })
