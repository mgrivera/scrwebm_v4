...
function ($scope, $stateParams, $state, $meteor, $modal, uiGridConstants) {
    ...
    $scope.helpers({
      ...
      $scope.refrescarGridCompanias = function() {
        ...
        $scope.reservas_ui_grid = {
            ...
            onRegisterApi: function (gridApi) {
                ...
                gridApi.selection.on.rowSelectionChanged($scope, function (row) {
                    ...
                    if (row.isSelected) {
                        reservaSeleccionada = row.entity;
                    }
                    else
                        return;
                });
        ...
        $scope.agregarRegistroReserva = function () {
            ...
            var ultimaReserva = $scope.siniestro.reservas[$scope.siniestro.reservas.length - 1];
        ...
        $scope.agregarRegistroLiquidaciones = function () {
            ...
            var ultimaLiquidacion = $scope.siniestro.liquidaciones[$scope.siniestro.liquidaciones.length - 1];
        ...
        $scope.construirCuotasMovimiento = function() {
            ...
            var existeNosotros = lodash.some($scope.siniestro.companias, function(c) { return c.nosotros; });
            ...
            let existenCuotasConCobrosAplicados = determinarSiExistenCuotasConCobrosAplicados(cuotasMovimientoSiniestro); 
            ...
            var cantidadCuotasLiquidacionSeleccionada = lodash($scope.cuotas).filter(function (c) {
  ...
  $scope.mostrarPagosEnCuota = function (cuota) {
      ...
      $scope.grabar = function () {
          ...
          if (item.docState != 3) {
              isValid = Siniestros.simpleSchema().namedContext().validate(item);
          ...
          if (item && item.companias && lodash.isArray(item.companias) && item.companias.length) {
              var errorMessage = "";
              var companiaNosotros = lodash.filter(item.companias, (c) => { return c.nosotros; });
              ...
              if (companiaNosotros.length == 0)
                  ...
                  // la compañía inidicada como 'nosotros' debe ser la misma que se registró en el catálogo
                  var companiaNosotrosEnCatálogo = Companias.findOne({ nosotros: true });

                  if (!companiaNosotrosEnCatálogo)
                      errorMessage = "Aparentemente, no existe una compañía del tipo 'nosotros' en el catálogo de compañías; " +
                                     "por favor registre una antes de intentar agregar un siniestro.";
                  else if (companiaNosotrosEnCatálogo._id != companiaNosotros[0].compania)
                      errorMessage = "La compañía del tipo 'nosotros' que Ud. indique en la lista, debe ser la misma " +
                                     "que la registrada en el <em>catálogo de compañías</em> para ese fin. Por favor revise.";
              };
        ...
        Meteor.call('siniestrosSave', item, (err, result)  => {
            ...
            Meteor.call('cuotasSave', cuotasArray, (err, result)  => {
                ...
                if (item.docState == 1) {
                    ...
                    filtro = JSON.stringify({ _id: item._id });
      ...
      $scope.mostrarEntidadDeOrigen = function() {
        ...
        var entidadOrigenID = $scope.siniestro.source && $scope.siniestro.source.entityID ?
                            $scope.siniestro.source.entityID :
                            0;

        var url2 = $state.href('riesgo', { origen: 'consulta', id: entidadOrigenID, pageNumber: 0, vieneDeAfuera: true });
