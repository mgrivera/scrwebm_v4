...
import './emailsCobranzaModal.html';
...
function ($scope, $modalInstance, $meteor, $modal, cuotasSeleccionadas) {
    ...
    $scope.submit_emailsCobranzaForm = function () {
        ...
        if ($scope.emailsCobranzaForm.$valid) {
            ...
            const editedItem = lodash.cloneDeep($scope.emailCobranza);
    ...
    $scope.asignarTiposDeCorreo = () => {
        ...
        const cuotasConEmailAsignado_array = lodash(cuotasSeleccionadas).
         ...
         $meteor.call('consultas_MontosPendientesCobro_GrabarTiposEmail', cuotasConEmailAsignado_array).then(
           ...
           function (err) {
               ...
               const errMessage = mensajeErrorDesdeMethod_preparar(err);
    ...
    $meteor.subscribe("emailsCobranzas").then(
        function() {
            ...
            if (!Array.isArray($scope.emailCobranza.usuarios)) {
                $scope.emailCobranza.usuarios = [];
            }

            if (!Array.isArray($scope.emailCobranza.firmantes)) {
                $scope.emailCobranza.firmantes = [];
            }

            if (!Array.isArray($scope.emailCobranza.cuentasBancarias)) {
                $scope.emailCobranza.cuentasBancarias = [];
            }

            if (!Array.isArray($scope.emailCobranza.reglas)) {
    ...
    $scope.nuevaRegla = function () {
        const count = $scope.emailCobranza.reglas.length;
