...
import { Temp_consulta_montosPagados, Temp_consulta_montosPagados2 } from '/imports/collections/consultas/temp_consulta_montosPagados';
import { Temp_consulta_montosPagados_config } from '/imports/collections/consultas/temp_consulta_montosPagados';
...
Meteor.methods(
    {
        'consulta.montosPagados': async function (filtro) {
            ...
            Temp_consulta_montosPagados.remove({ user: this.userId });
            Temp_consulta_montosPagados2.remove({ user: this.userId });
            ...
            match['$and'].push({ 'pagos.monto': { $gt: 0 } });               // solo cobros (monto negativo)

            if (filtro.periodoPagos1) {
                match['$and'].push({ 'pagos.fecha': { $gte: filtro.periodoPagos1 } });     // solo cobros para el período indicado 
            }

            if (filtro.periodoPagos2) {
                match['$and'].push({ 'pagos.fecha': { $lte: filtro.periodoPagos2 } });     // solo cobros para el período indicado 
            }
            ...
            // preparamos un 2do filtro para *omitir* pagos que no correspondan al período. Ahora, el filtro puede traer 
            // pagos con varios items, aunque no es frecuente. Algunos de esos pagos pueden *no* corresponder al 
            // período indicado 
            const match2 = { $and: [] };

            match2['$and'].push({ 'pagos.monto': { $gt: 0 } });               // solo cobros (monto positivo)

            if (filtro.periodoPagos1) {
                match2['$and'].push({ 'pagos.fecha': { $gte: filtro.periodoPagos1 } });     // solo cobros para el período indicado 
            }

            if (filtro.periodoPagos2) {
                match2['$and'].push({ 'pagos.fecha': { $lte: filtro.periodoPagos2 } });     // solo cobros para el período indicado 
            }

            if (filtro.monedas && Array.isArray(filtro.monedas) && filtro.monedas.length) {     // el usuario puede filtrar por moneda
                // nótese que el usuario puede, para un mismo monto original, registrar más de un pago, pero para monedas diferentes  
            ...
            for (const cobro of cobros) {
                ...
                Temp_consulta_montosPagados.insert(cobro);
            }
            ...
            const items = Temp_consulta_montosPagados.find({ user: this.userId }, { fields: { _id: 1, source: 1 } }).fetch();
            ...
            for (const item of items) {
                ...
                Temp_consulta_montosPagados.update(item._id, { $set: { ramo: parentEntity.ramo, asegurado: parentEntity.asegurado } },
                    { multi: false });
            }
            ...
            pipeline = [
                ...
                { $merge: { into: "temp_consulta_montosPagados2", on: "_id", whenMatched: "replace", whenNotMatched: "insert" } }
            ]

            await Temp_consulta_montosPagados.rawCollection().aggregate(pipeline).toArray();

            // finalmente, agregamos un asegurado vacío cuando no hay uno (caso contratos) 
            Temp_consulta_montosPagados2.update({ user: this.userId, asegurado: { $exists: 0 } },
        ...
        'consultas.montosPagados.getRecCount': function (userId) {

            check(userId, String);

            const recordCount = Temp_consulta_montosPagados2.find({ user: userId }).count();
        ...
        'consulta.montosPagados.report.grabarAMongoOpcionesReporte': function (opcionesReporte, empresaSeleccionada) {
            ...
            Temp_consulta_montosPagados_config.remove({ user: Meteor.userId() });
            ...
            Temp_consulta_montosPagados_config.insert({
...
function prepararFechasEnFiltro(filtro) {
    ...
    f.periodoPagos1 = moment(f.periodoPagos1).isValid() ? moment(f.periodoPagos1).toDate() : null;
    f.periodoPagos2 = moment(f.periodoPagos2).isValid() ? moment(f.periodoPagos2).toDate() : null;

    // la fecha final del período debe ser el último momento del día, para que incluya cualquier fecha de ese día 
    f.periodoPagos2 = f.periodoPagos2 ? new Date(f.periodoPagos2.getFullYear(), f.periodoPagos2.getMonth(), f.periodoPagos2.getDate(), 23, 59, 59) : null;
