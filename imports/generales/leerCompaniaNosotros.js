...
// para el usuario 
export const LeerCompaniaNosotros = function(userID) { 

    const empresaUsuariaSeleccionada = CompaniaSeleccionada.findOne({ userID: userID });

    if (!empresaUsuariaSeleccionada) { 
        const message = `Error (inesperado): no hemos podido leer una empresa usuaria seleccionada para el usuario.`
        return { 
            error: true, 
            message: message
        }
    ...
    if (!empresaUsuariaSeleccionada.companiaNosotros) { 
        const message = `Error: la <em>empresa usuaria</em> seleccionada para el usuario, no tiene un compañía del tipo 'nosotros' asociada.<br /> 
    ...
    const companiaNosotros = Companias.findOne(empresaUsuariaSeleccionada.companiaNosotros); 

    if (!companiaNosotros) { 
        const message = `Error (inesperado): no hemos podido leer, en el catálogo de Compañías, la compañía que corresponde a la compañía 
        'nosotros'.<br /> 
