...
export function ReportPrintModal({ reportPrintModalShow, setReportPrintModalShow, companiaSeleccionada }) { 
    ...
    useEffect(() => {
        ...
        const filtroAnterior = Filtros.findOne(
            {
                nombre: 'consultas.cumulos.config',
    ...
    const guardarOpciones = () => {
        ...
        Meteor.call('consultas.cumulos.report.grabarAMongoOpcionesReporte', formValues, companiaSeleccionada,
            (err, result) => {
    ...
    if (scrwebm_net_app_address) {
        reportLink = `${scrwebm_net_app_address}/reports/consultas/cumulos/report.aspx?user=${Meteor.userId()}&report=cumulos`;
    }
    
    return (
        <Modal show={reportPrintModalShow} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>Cúmulos - Consulta</Modal.Title>
            </Modal.Header>
            <Modal.Body>
...
function guardarFiltro(values) {
    ...
    const filtro = Filtros.findOne({ nombre: 'consultas.cumulos.config', userId: Meteor.userId() }, { fields: { _id: 1 } });
    if (filtro) {
