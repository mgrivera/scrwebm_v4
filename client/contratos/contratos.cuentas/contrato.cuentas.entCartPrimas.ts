                ...
                $scope.contratosProp_entCartPr_resumen
                    ...
                    .forEach((x) => { 
                        if (x.docState && x.docState === 1) { 
                            lodash.remove($scope.contratosProp_entCartPr_resumen, (y: any) => { return y._id === x._id; });
                        } else { 
                            $scope.contratosProp_entCartPr_resumen.find(y => y._id === x._id).docState = 3; 
                        }
                    })

                result.resumenPrimasSiniestros_array.forEach((x) => {
                    let resumenEntCartPr_item = {
                    ...
                    $scope.contratosProp_entCartPr_resumen.push(resumenEntCartPr_item);
                })
