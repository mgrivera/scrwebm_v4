                ...
                $scope.contratosProp_entCartSn_resumen
                    ...
                    .forEach((x) => { 
                        if (x.docState && x.docState === 1) { 
                            lodash.remove($scope.contratosProp_entCartSn_resumen, (y: any) => { return y._id === x._id; });
                        } else { 
                            $scope.contratosProp_entCartSn_resumen.find(y => y._id === x._id).docState = 3; 
                        }
                    })

                result.resumenPrimasSiniestros_array.forEach((x) => {
                    let resumenEntCartSn_item = {
                    ...
                    $scope.contratosProp_entCartSn_resumen.push(resumenEntCartSn_item);
                })
