...
angular.module("scrwebM").controller("Cierre.Registro.Controller", ['$scope', '$modal', '$interval', function ($scope, $modal, $interval) {
    ...
    let registro_ui_grid_api = {} as any;
    ...
    $scope.deleteItem = function (item) {
        ...
        if (item.docState && item.docState === 1) {
            ...
            lodash.remove($scope.registro, (x: any) => { return x._id === item._id; });
        }
        else {
            item.docState = 3;
        }
                ...
                if (!isValid) {
                    CierreRegistro.simpleSchema().namedContext().validationErrors().forEach((error) => {
                        if (error.type === "custom") { 
                            ...
                            errores.push(`El valor '${error.value}' no es adecuado para el campo '${CierreRegistro.simpleSchema().label(error.name)}'; error de tipo '${error.type}'.` as never);
                        }
                    });
                }
            }
