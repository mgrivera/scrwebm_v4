...
export function ReportPrintModal({ reportPrintModalShow, setReportPrintModalShow, companiaSeleccionada }) { 
    ...
    useEffect(() => {
        ...
        const filtroAnterior = Filtros.findOne(
            {
                nombre: 'consultas.montosPagados.config',
    ...
    const guardarOpciones = () => {
        ...
        Meteor.call('consulta.montosPagados.report.grabarAMongoOpcionesReporte', formValues, companiaSeleccionada,
            (err, result) => {
    ...
    if (scrwebm_net_app_address) {
        reportLink = `${scrwebm_net_app_address}/reports/consultas/montosPagados/report.aspx?user=${Meteor.userId()}&report=montosPagados`;
    }
    
    return (
        <Modal show={reportPrintModalShow} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>Montos pagados - Consulta</Modal.Title>
            </Modal.Header>
            <Modal.Body>
...
function guardarFiltro(values) {
    ...
    const filtro = Filtros.findOne({ nombre: 'consultas.montosPagados.config', userId: Meteor.userId() }, { fields: { _id: 1 } });
    if (filtro) {
