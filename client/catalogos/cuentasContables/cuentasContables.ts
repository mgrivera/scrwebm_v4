...
angular.module("scrwebM").controller("CuentasContablesController", ['$scope', '$modal', function ($scope, $modal) {
    ...
    $scope.cuentasContables_ui_grid.columnDefs = [
      ...
      // subscriptions ...
      subscriptionHandle =
      Meteor.subscribe('cuentasContables', () => {
      ...
      $scope.deleteItem = function (item) {
          if (item.docState && item.docState === 1) {
              ...
              lodash.remove($scope.cuentasContables, (x: any) => { return x._id === item._id; });
          }
              ...
              if (item.docState != 3) {
                  ...
                  if (!isValid) {
                      CuentasContables.simpleSchema().namedContext().validationErrors().forEach(function (error) {
                          errores.push("El valor '" + error.value + "' no es adecuado para el campo '" + CuentasContables.simpleSchema().label(error.name) + "'; error de tipo '" + error.type + "'." as never);
                      });
                  }
              }
          })
