      ...
      $scope.$meteorSubscribe('remesas', JSON.stringify({ _id: remesaID })).then(function(subscriptionHandle) {
            ...
            $scope.$meteorSubscribe('cuotas', JSON.stringify(filtro)).then(function(subscriptionHandle) {
                    ...
                    cuotas.forEach(function(cuota) {
                        _.filter(cuota.pagos, function(x) { return x.remesaID === remesaID; }).forEach(function(p) {
                            var pago = {};
                    ...
                    var promise = DialogModal($modal, "<em>Cobranzas</em>",
                                                      "Ok, el proceso fue ejecutado en forma satisfactoria.<br />" +
                                                      "En total fueron aplicados <b>" + cantidadPagosAplicados.toString() + "</b> cobros/pagos a la remesa seleccionada.<br /><br />" +
                                                      "En cualquier momento, Ud. puede revisar los pagos aplicados, si consulta la remesa y hace un <em>click</em> en la opción <em>Detalles</em>.<br /><br />" +
                                                      "También puede <em>revertir</em> esta operación, si consulta la remesa y hace un <em>click</em> en la opción <em>Revertir</em>.",
                                                      false).then();
                     $scope.showProgress = false;
                 });
             });
