...
angular.module("scrwebm").controller("CuentasContablesAsociadas_Controller", ['$scope', function ($scope) {
    ...
    $scope.deleteItem = function (item) {
        if (item.docState && item.docState === 1) {
            ...
            lodash.remove($scope.cuentasContablesAsociadas, (x: any) => { return x._id === item._id; });
        }
        ...
        editedItems.forEach(function (item) {
            if (item.docState != 3) {
                ...
                CuentasContablesAsociadas.simpleSchema().namedContext().validationErrors().forEach(function (error) {
                        errores.push("El valor '" + error.value + "' no es adecuado para el campo '" + CuentasContablesAsociadas.simpleSchema().label(error.name) + "'; error de tipo '" + error.type + "'." as never);
                    });
                }
            }
        })
