...
angular.module("scrwebM").controller("TiposFacultativoController", ['$scope', function ($scope) {
      ...
      // ---------------------------------------------------------
      // subscriptions ...
      Meteor.subscribe('tiposFacultativo', () => { 
      ...
      $scope.save = function () {
          ...
          var editedItems = lodash.filter($scope.tiposFacultativo, function (item) { return item.docState; });
              ...
              if (item.docState != 3) {
                  ...
                  if (!isValid) {
                      TiposFacultativo.simpleSchema().namedContext().validationErrors().forEach(function (error) {
                          errores.push("El valor '" + error.value + "' no es adecuado para el campo '" + TiposFacultativo.simpleSchema().label(error.name) + "'; error de tipo '" + error.type + "'." as never);
                      });
                  }
              }
          })
