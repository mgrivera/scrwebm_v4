...
export function mongoCollection_array_grabar(mongoCollectionArray, mongoCollection) { 
    ...
    const inserts = mongoCollectionArray.filter((x) => { return x.docState && x.docState == 1; })
    ...
    const updates = mongoCollectionArray.filter((item) => { return item.docState && item.docState == 2; })
    ...
    const removes = mongoCollectionArray.filter((x) => { return x.docState && x.docState == 3; });
